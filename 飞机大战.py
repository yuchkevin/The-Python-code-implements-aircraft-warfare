...
#根绝得分增加难度
def addLevelByScore(level, score):
...
def main():
    ...
    pause_nor_image = pygame.image.load(
        "images/pause_nor.png").convert_alpha()  # 暂停按钮
    ...
    paused_rect.left, paused_rect.top = width - \
        paused_rect.width - 10, 10  # 距离左边和上边各10个距离
    ...
    # Pygame 中用于定时触发自定义事件的核心函数， 这个时候注册了，后面循环才能捕捉到
    # #在你的飞机大战代码中，它的作用是每 30 秒自动生成一个补给包
    ...
    while True:  # 循环条件
        ...
        # 事件处理完成
        # 根据用户的得分增加难度
        level=addLevelByScore (level, score)
